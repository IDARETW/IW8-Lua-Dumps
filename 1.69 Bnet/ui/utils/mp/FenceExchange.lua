...
function FenceExchange.UpdateState(arg_7_0)
	...
	for iter_7_0 = 0, Engine.DCJGJDDEHE() - 1 do
		...
		if var_7_0 and not var_7_1 then
			if Commerce.CIIJICHHF(iter_7_0) == FenceExchange.SignInExchangeState.ERRORS then
				Commerce.DBFDIGCEID(iter_7_0, FenceExchange.SignInExchangeState.SHOWN_POPUP)
			end

			if Commerce.CIIJICHHF(iter_7_0) <= FenceExchange.SignInExchangeState.WORKING then

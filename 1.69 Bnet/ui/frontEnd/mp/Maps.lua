...
local function var_0_4(arg_3_0, arg_3_1, arg_3_2)
	...
	local var_3_0 = {}
	local var_3_1 = Dvar.IBEGCHEFE("MQQLQQRM")

	if var_3_1 then
		for iter_3_0, iter_3_1 in pairs(Lobby.gameModeCategories) do
			local var_3_2 = Lobby.GetModesForCategory(iter_3_1)

			for iter_3_2, iter_3_3 in ipairs(var_3_2) do
				if iter_3_3.ref ~= nil then
					var_3_0[iter_3_3.ref] = true
				end
			end
		end
	end

	local var_3_3 = Lobby.GetCurrentGameType()
	local var_3_4 = {}

	for iter_3_4 = 0, Lobby.DEFCCIGHCF() - 1 do
		local var_3_5 = Lobby.CJEHDHDGBH(iter_3_4)
		local var_3_6 = PRIVATE_MATCH.ShouldDisplayMap(iter_3_4, arg_3_1) and PRIVATE_MATCH.MapSupportsCustomGames(var_3_5)

		if var_3_1 and var_3_6 then
			var_3_6 = false

			for iter_3_5 in Lobby.DIIFDCEJJ(iter_3_4):gmatch("%w+") do
				if var_3_0[iter_3_5] ~= nil then
					var_3_6 = true
		...
		if var_3_6 then
			local var_3_7 = Lobby.CBEIEIFCEF(iter_3_4)
			local var_3_8 = Lobby.DIIFDCEJJ(iter_3_4)
			local var_3_9 = {
				isExtendedMode = false,
				name = Engine.CBBHFCGDIC(StringTable.DIFCHIGDFB(CSV.mpMapTable.file, CSV.mpMapTable.cols.ref, var_3_5, CSV.mpMapTable.cols.name)),
				ref = var_3_5,
				desc = Lobby.BGCBFAABHB(iter_3_4),
				image = MP.GetMapImage(Lobby.CJEHDHDGBH(iter_3_4)),
				gameTypes = var_3_8,
				disabled = not string.match(var_3_8, var_3_3) and not Dvar.IBEGCHEFE("LSSRRSMNMR"),
				isOwned = Engine.BAJHDFAJJF(var_3_7)
			}

			if PRIVATE_MATCH.DoesMapNeedsMultipleModes(var_3_5) then
				var_0_2(var_3_4, var_3_9)
			else
				table.insert(var_3_4, var_3_9)
			end
		end
	end

	if LUI.FlowManager.GetScopedData(arg_3_0).isTournament then
		local var_3_10 = Lobby.RANDOM_MAP
		local var_3_11 = {
			disable = false,
			isOwned = true,
			desc = "",
			name = MP.GetLobbyMapName(var_3_10),
			ref = var_3_10,
			image = MP.GetLobbyMapImage(var_3_10),
			gameTypes = {}
		}
	...
	local var_3_12 = LUI.DataSourceFromList.new(#var_3_4)

	function var_3_12.MakeDataSourceAtIndex(arg_5_0, arg_5_1)
		local var_5_0 = var_0_0 .. ".map." .. arg_5_1 + 1
		local var_5_1 = var_3_4[arg_5_1 + 1]
	...
	local var_3_13 = LUI.DataSourceInControllerModel.new("alwaysLoaded.isExtendedModeSelected")
	local var_3_14 = var_3_13:GetValue(arg_3_1)

	if var_3_14 == nil then
		var_3_13:SetValue(arg_3_1, Dvar.CFHDGABACF("scr_map_use10v10_objectives") == 1)
	end

	function var_3_12.GetDefaultFocusIndex()
		local var_6_0 = Dvar.DHEEJCCJBH("NSQLTTMRMP")

		for iter_6_0 = 0, var_3_12:GetCountValue(arg_3_1) - 1 do
			if var_3_12:GetDataSourceAtIndex(iter_6_0).ref:GetValue(arg_3_1) == var_6_0 then
				local var_6_1 = var_3_12:GetDataSourceAtIndex(iter_6_0).isExtendedMode:GetValue(arg_3_1)

				if not var_6_1 or var_6_1 and var_3_14 then
					return {
						x = 0,
						y = iter_6_0
					}
	...
	arg_3_0:SetDataSource(var_3_12, arg_3_1)

	if Engine.ECHCFGDDDF() then
		local var_3_15 = LUI.UIElement.new()

		var_3_15:SetBlurStrength(5)
		var_3_15:setupWorldBlur()

		var_3_15.id = "blur"

		arg_3_0:addElement(var_3_15)
	end

	local var_3_16 = arg_3_0.Maps:GetFirstFocusablePosition()

	if LUI.IsLastInputKeyboardMouse(arg_3_1) and var_3_16 then
		arg_3_0.Maps:SetFocusedPosition(var_3_16, true)
	end
end

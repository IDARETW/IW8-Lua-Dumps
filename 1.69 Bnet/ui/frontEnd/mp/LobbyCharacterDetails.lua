...
local function var_0_1(arg_2_0, arg_2_1)
	arg_2_0.Gamertag:setText(arg_2_1.fullName)
	...
	if arg_2_1.rank then
		local var_2_0 = tonumber(arg_2_1.rank)

		if var_2_0 then
			assert(var_2_0 >= 0, "LobbyCharacterDetails->Setup(), combinedRank is negative but expected to never be, verify what the backend is sending down now")

			local var_2_1 = PROGRESSION.GetPlayerProgressionData({
				_combinedRankNum = var_2_0,
				_prestigeLevel = arg_2_1.prestige
			})

			arg_2_0.Rank:setText(var_2_1._rankDisplay)
			arg_2_0.RankIcon:setImage(RegisterMaterial(var_2_1._rankIcon))
			ACTIONS.AnimateSequence(arg_2_0, "StandardRank")
		end
	end
	...
	local var_2_2 = arg_2_1.presencePlatform ~= nil and arg_2_1.presencePlatform > 0
	...
	if arg_2_1.isMyPlayer then
		...
		if arg_2_1.usingGamepad ~= nil then
			arg_2_0:dispatchEventToRoot({
				name = "self_using_gamepad_update",
				usingGamepad = arg_2_1.usingGamepad
			})
		end
	else
...
function LobbyCharacterDetails(arg_4_0, arg_4_1)
	...
	var_4_10:setImage(RegisterMaterial("icon_t9_rank_01"), 0)

...
local function var_0_4(arg_4_0, arg_4_1)
	...
	if Dvar.IBEGCHEFE("LQSTPOPKRO") then
		local var_4_1 = Engine.EAHFCIGDIG(var_4_0)
		local var_4_2 = Engine.DDGCFIDIHJ(var_4_0)
		local var_4_3 = Engine.JBFICJJHG(var_4_0)
		local var_4_4 = Engine.HFFCCEIAB(var_4_0)

		if var_4_1 or var_4_2 or var_4_3 or var_4_4 then
			local var_4_5 = false
			local var_4_6 = Engine.DBFFAEEFGJ()

			if var_4_6 ~= nil then
				local var_4_7 = var_4_6.flowManager:GetStackTop(true)

				if var_4_7 and var_4_7.name == "BRMainMenu" then
					var_4_5 = true
				end
			end

			local var_4_8 = Engine.BECCFCBIAA("hasSeenCOPPANoticeMessage", var_4_0) == 1

			if var_4_5 and not var_4_8 then
				Dvar.DHEGHJJJHI("cl_voice", false)
				Lobby.HCCAGAJGJ(var_4_0, MuteStrategy.MuteStrategyMuteAll)
				Engine.DFJDIFJEGA("textChatEnabled", 0, var_4_0)
				Dvar.DHEGHJJJHI("LKSKPKTOON", false)
				LUI.FlowManager.RequestPopupMenu(arg_4_0, "COPPAMenuNotice", true, var_4_0, false, nil, nil, true, true)
			end
		end
	end
	...
	if CONDITIONS.IsSeasonalEventActive() then
		local var_4_9 = SEASONAL_EVENT.GetCurrentSeasonalEventOnboadingData()

		if var_4_9 and not var_4_9.WasCompleted(arg_4_0, var_4_0) then
			...
			local var_4_10 = SEASONAL_EVENT.GetCurrentSeasonalEventData()

			if not CONDITIONS.SeasonalEventDisablesFTUE() then
				if var_4_10.onboardingBink == nil or var_4_10.onboardingBink.isKorea15Unsafe and Engine.FABABBDBA() then

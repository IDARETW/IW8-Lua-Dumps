...
local function var_0_0(arg_1_0, arg_1_1)
	...
	if arg_1_1.price and not arg_1_1.owned then
		if arg_1_1.price == "" then
			arg_1_0.PriceBackground:SetAlpha(0)
		else
...
local function var_0_1(arg_2_0, arg_2_1, arg_2_2)
	...
	arg_2_0:addEventHandler("button_action", function(arg_3_0, arg_3_1)
		...
		if arg_2_0._buttonData.isFirstParty then
			...
			elseif STORE.IsZeusProduct(var_3_1) or STORE.IsZeusTrialProduct(var_3_1) then
				LUI.FlowManager.RequestAddMenu("StoreGameUpsellPopup", true, arg_2_1, false, {
					preFetched = true,
					isZeus = true,
					defaultProductId = var_3_1
				}, true)
			elseif STORE.IsVanguardProduct(var_3_1) then
				if var_3_1 == STORE.VanguardTrialID then
					if Engine.DBFCJBDJEC() then
						STORE.GoToX2InBattleNet(arg_2_1)
					elseif arg_2_0._buttonData.uri ~= nil then
						Engine.CBEBCGFAIE(arg_2_1, arg_2_0._buttonData.uri)
					else
						InGameStore.SelectProduct(arg_2_1, var_3_1, InGameStorePurchaseType.BROWSE)
					end
				else
			...
			elseif STORE.IsCortezProduct(var_3_1) then
				LUI.FlowManager.RequestPopupMenu(arg_2_0, "CortezPreOrderPopup", true, arg_2_0._controllerIndex, false, {
					defaultProductId = var_3_1,
					cortezEntryPointMenu = STORE.CortezEntryPointMenu.FranchiseStore
				})
			elseif var_3_0 then

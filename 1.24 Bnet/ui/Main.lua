...
function OpenInitialMenu()
	...
	elseif Engine.DDJFBBJAIG() and var_5_3 then
		...
		if not var_5_1 or Engine.ECAJCAJJGF() or not var_5_2 then
			...
			elseif Dvar.IBEGCHEFE("LNQLRSNMQS") then
				if CONDITIONS.IsMagmaGameMode() then
					...
					if var_5_8 == MP.WarzonePracticePopupStatus.GO_TO_WARZONE_LOBBY or var_5_8 == MP.WarzonePracticePopupStatus.GO_TO_BLOODMONEY_LOBBY then
						...
						if var_5_8 == MP.WarzonePracticePopupStatus.GO_TO_PRACTICE_LOBBY and Dvar.IBEGCHEFE("MNRTKTQMLS") then
							LUI.FlowManager.RequestAddMenu("LobbyWarzoneTutorial", true, var_5_5, false, {
	...
	elseif Engine.DDJFBBJAIG() and Engine.CAADCDEEIA() then
		if not Engine.HDDHJIBDH() then
			local var_5_11 = Engine.IJEBHJIJF()

			if Engine.HABIAFADE() then
				LUI.FlowManager.RequestRestoreMenu("CampaignMenu", nil, var_5_11)
			else
				LUI.FlowManager.RequestRestoreMenu("CampaignMenuOffline", nil, var_5_11)
			end
		end
	elseif Engine.CAADCDEEIA() and not Engine.DDJFBBJAIG() and Dvar.IBEGCHEFE("NLONRMOLKS") then
		local var_5_12 = Engine.IJEBHJIJF()

		LUI.FlowManager.RequestRestoreMenu("pre_game", nil, var_5_12, nil, nil, true)
	end
end
...
function Main()
	...
	CoD.UpdateGameMode(Engine.DGEFGAJIB(), CoD.PlayMode.None)
	OpenInitialMenu()
end
